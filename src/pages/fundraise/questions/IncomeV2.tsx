...
export default function QuestionIncomeV2() {
    const navigate = useNavigate();
    const { update } = useFundrise();

    const handleSelect = (value: string) => {
        update({ income: value });
        navigate('/questions/satisfied-investment');
    };

    return (
        <FundriseShell
            onBack={() => navigate('/questions/motivation')}
            onClose={() => navigate('/')}
        >
            <div className="bg-white rounded-xl border border-[#EEE9E0] shadow-[0_2px_20px_rgba(21,21,19,0.06)] px-6 py-8 sm:px-10">
                ...
                <p className="text-[14px] text-[#8C8479] mb-8">Please use your individual pre-tax income.</p>
